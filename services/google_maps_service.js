...
function showAlertsList (currentList) {
  ...
  const cityNames = currentList.weather.map(elem => { return elem.cityName })
  const alerts = currentList.weather.map((elem, idx) => { return elem.alerts ? { city: cityNames[idx], alert: elem.alerts[0] } : undefined }).filter(elem => { return elem })
...
function renderForecastDays (dailies) {
  ...
  dailies.forEach(function (period) {
    ...
    const template = (`
          <div class="col-6 col-md-3 col-lg-2 d-flex my-2">
